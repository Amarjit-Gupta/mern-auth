...
const AllData = () => {
    ...
    const handleSearch = async (e) => {
        ...
        if (key) {
            let result = await fetch(`${DATAURL}/data/search/${key}`);
            let data = await result.json();
    ...
    const handleDelete = async (index) => {
        ...
        if (delData) {
            let result = await fetch(`${DATAURL}/data/deleteSingleData/${index}`, {
                method: "delete"
            });
            let data = await result.json();
            if (data.success) {
                                ...
                                value.map((v, i) => {
                                    return (
                                        <div className="card" key={v._id}>
                                            <div className="card-child1"><img src={`${DATAURL}/uploads/${v.file}`} alt="broken image" /></div>
